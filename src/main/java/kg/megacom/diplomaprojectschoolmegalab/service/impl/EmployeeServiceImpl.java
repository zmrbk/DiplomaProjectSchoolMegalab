...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public void createEmployee(EmployeeCreateRequest request) {
        userService.getById(request.getUserId()).orElseThrow(()-> new RuntimeException("User not found, please create new user first"));
    ...
    public Response findEmployeeById(Long id) {
        ...
        EmployeeCreateRequest employeeCreateRequest = employeeMapper.toStaffCreateRequest(employee);
    ...
    public Response updateEmployee(EmployeeCreateRequest request, Long id) {
        ...
        Employee oldEmployee = employeeRepository.findById(id).orElseThrow(()-> new RuntimeException("Staff not found"));
        Employee newEmployee = employeeMapper.toEmployee(request, oldEmployee);
        newEmployee.setId(id);
        employeeRepository.save(newEmployee);
        return new Response("Update Employee", employeeMapper.toStaffCreateRequest(newEmployee));
    }

    @Override
    public Response getAllEmployees() {
        return new Response("Get all Employee", employeeMapper.toStaffCreateRequestList(employeeRepository.findAll()));
    }
